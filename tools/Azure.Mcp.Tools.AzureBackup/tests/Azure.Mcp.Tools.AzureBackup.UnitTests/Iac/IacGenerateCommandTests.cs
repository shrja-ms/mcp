...
namespace Azure.Mcp.Tools.AzureBackup.UnitTests.Iac;

public class IacGenerateCommandTests
{
    private readonly IAzureBackupService _backupService;
    private readonly IacGenerateCommand _command;
    ...
    public IacGenerateCommandTests()
    {
        ...
        var logger = Substitute.For<ILogger<IacGenerateCommand>>();
    ...
    [Fact]
    public async Task ExecuteAsync_GeneratesIac_Successfully()
    {
        var expectedResult = new OperationResult("Succeeded", null, "Operation completed");
        _backupService.GenerateIacFromVaultAsync(
        ...
        var args = _commandDefinition.Parse(["--subscription", "sub123", "--resource-group", "rg1", "--vault", "vault1", "--iac-format", "terraform"]);
        var response = await _command.ExecuteAsync(_context, args, TestContext.Current.CancellationToken);

        Assert.Equal(HttpStatusCode.OK, response.Status);
        var json = JsonSerializer.Serialize(response.Results);
        var result = JsonSerializer.Deserialize(json, AzureBackupJsonContext.Default.IacGenerateCommandResult);
