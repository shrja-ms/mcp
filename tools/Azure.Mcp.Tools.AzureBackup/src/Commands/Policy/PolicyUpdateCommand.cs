...
public sealed class PolicyUpdateCommand(ILogger<PolicyUpdateCommand> logger) : BaseAzureBackupCommand<PolicyUpdateOptions>()
{
    private const string CommandTitle = "Update Backup Policy";
    private readonly ILogger<PolicyUpdateCommand> _logger = logger;

    public override string Id => "b1a2c3d4-e5f6-7890-abcd-ef12345678a3";
    public override string Name => "update";
    public override string Description => "Updates an existing backup policy's schedule and retention rules.";
    ...
    public override async Task<CommandResponse> ExecuteAsync(CommandContext context, ParseResult parseResult, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = await service.UpdatePolicyAsync(options.Vault!, options.ResourceGroup!, options.Subscription!, options.Policy!, options.VaultType, options.ScheduleFrequency, options.DailyRetentionDays, options.WeeklyRetentionWeeks, options.Tenant, options.RetryPolicy, cancellationToken);
            context.Response.Results = ResponseResult.Create(new PolicyUpdateCommandResult(result), AzureBackupJsonContext.Default.PolicyUpdateCommandResult);
        }
        catch (Exception ex) { _logger.LogError(ex, "Error updating policy"); HandleException(context, ex); }
        return context.Response;
    }

    internal record PolicyUpdateCommandResult([property: JsonPropertyName("result")] OperationResult Result);
}
