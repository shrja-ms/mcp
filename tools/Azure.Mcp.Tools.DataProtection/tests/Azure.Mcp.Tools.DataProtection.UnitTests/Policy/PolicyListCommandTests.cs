...
public class PolicyListCommandTests
{
    ...
    [Fact]
    public async Task ExecuteAsync_ReturnsPolicies_WhenPoliciesExist()
    {
        var expected = new BackupPolicyModel[] { new() { Name = "policy1" }, new() { Name = "policy2" } };
        _service.ListPoliciesAsync("vault1", "rg1", "sub123", Arg.Any<string?>(), Arg.Any<RetryPolicyOptions?>(), Arg.Any<CancellationToken>())
            .Returns(expected);

        var command = new PolicyListCommand(_logger);
        ...
        var result = JsonSerializer.Deserialize(json, DataProtectionJsonContext.Default.PolicyListCommandResult);
    ...
    {
        _service.ListPoliciesAsync("vault1", "rg1", "sub123", Arg.Any<string?>(), Arg.Any<RetryPolicyOptions?>(), Arg.Any<CancellationToken>())
            .Returns(Array.Empty<BackupPolicyModel>());
